...
public:
    ...
    void start()
    {
        ...
        mRunning = true;
        mThread = ThreadRef( new std::thread( &ThreadedFaceDetector::run, this ) );
        mEnlargementValue= 0.4;
    }
    void stop()
    {
        mRunning = false;
    ...
    void run()
    {
        while ( mRunning ) {
            ...
            for( vector<cv::Rect>::const_iterator faceIter = faces.begin(); faceIter != faces.end(); ++faceIter ) {
                ...
                mHeads.push_back( enlargeFaceBox(mEnlargementValue, faceRect));
